...
use smb_proto::auth::spnego::{OID_NTLMSSP, OID_SPNEGO};
use smb_proto::framing::{decode_frame_header, encode_frame, FRAME_HEADER_LEN};
use smb_proto::header::{Command, HeaderTail, Smb2Header, SMB2_FLAGS_SERVER_TO_REDIR};
...
pub async fn read_frame(s: &mut TcpStream) -> Vec<u8> {
    ...
    let len = decode_frame_header(&hdr).expect("decode hdr");
